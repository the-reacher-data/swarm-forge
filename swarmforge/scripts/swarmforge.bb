...
(def legacy-backends #{"claude" "codex" "copilot" "grok"})

(defn backend-names-in [path]
  (if (fs/exists? path)
    (->> (str/split-lines (slurp (str path)))
         (keep #(second (re-matches #"\s*\[instances\.([A-Za-z0-9_-]+)\]\s*" %)))
         set)
    #{}))

(defn configured-backends [ctx]
  (into legacy-backends
        (concat (backend-names-in (:backends-file ctx))
                (backend-names-in (:local-backends-file ctx)))))
          ...
          (if (or (str/blank? line) (str/starts-with? line "#"))
            ...
            (let [fields (str/split line #"\s+")]
              ...
              (let [[keyword role agent worktree & trailing] fields
                ...
                (when-not (#{"window" "lazy-window"} keyword)
                ...
                (when-not (contains? (configured-backends ctx) agent)
        ...
        (apply str
               (for [row (:roles ctx)]
                 (format "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
                         ...
                         (:agent row)
                         (:receive-mode row)
                         (if (:lazy row) "lazy" "eager"))))))

(def required-helpers
  ["agent_catalog.py" "backend_runner.py" "handoff_lib.bb" "swarm_handoff.sh" "swarm_handoff.bb"
...
(defn check-backend-dependencies! [ctx]
  (check-dependency! "python3")
  (doseq [agent (distinct (map :agent (:roles ctx)))]
    (let [result (process/sh {:continue true}
                             "python3" (str (fs/path (:script-dir ctx) "backend_runner.py"))
                             "--root" (str (:working-dir ctx))
                             "--instance" agent
                             "--check")]
      (when-not (zero? (:exit result))
        (fail! (str red "Error:" reset " " (str/trim (:err result))))))))
  ...
  (let [role (:role row)
    ...
    (cond-> (str base
                (case agent
                  ...
                  (str "python3 " (sq (str (fs/path role-script-dir "backend_runner.py")))
                       " --root " (sq (str (:working-dir ctx)))
                       " --instance " (sq agent)
                       " --role " (sq role)
                       " --display " (sq display)
                       " --worktree " (sq (str role-worktree))
                       " --prompt " (sq (str prompt-file))
                       " --"
                       (apply str (map #(str " " (sq %)) (:extra-arg-tokens row))))))
      (= index 0)
...
(defn prepare-launch-state! [ctx]
  (let [launch-dir (fs/path (:state-dir ctx) "launch")
        started-dir (fs/path (:state-dir ctx) "started")]
...
(defn context [working-dir]
  (let [working-dir (fs/absolutize (fs/path working-dir))
    ...
    {:working-dir working-dir
     ...
     :backends-file (fs/path swarm-forge-dir "backends.toml")
     :local-backends-file (fs/path state-dir "backends.local.toml")
      ...
      (let [ctx (assoc ctx :terminal-backend (detect-terminal-backend))]
        ...
        (let [delay-ms (env-long "SWARMFORGE_AGENT_START_DELAY_MS" 1500)]
          (doseq [[index row] (map-indexed vector (:roles ctx))
            ...
            (spit (str (fs/path (:state-dir ctx) "started" (:role row))) "started\n")))
        (println)
