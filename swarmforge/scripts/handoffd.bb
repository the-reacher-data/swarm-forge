...
(defn load-roles []
  (into {}
        (for [line (read-lines roles-file)
              ...
              :let [[role worktree-name worktree-path session display agent receive-mode startup]
                    (str/split line #"\t")]]
          [role {:role role
...
(defn render-message [headers body]
  (let [preferred ["id" "from" "to" "recipient" "priority" "type" "role" "task"
                   "base_commit" "outcome" "commit"
...
(defn ensure-started! [socket role-info]
  (when (= "lazy" (:startup role-info))
    (let [role (:role role-info)
          marker (fs/path state-dir "started" role)]
      (when-not (fs/exists? marker)
        (let [launch-file (fs/path state-dir "launch" role)
              command (when (fs/exists? launch-file)
                        (str/trim (slurp (str launch-file))))]
          (when (str/blank? command)
            (throw (ex-info (str "missing lazy launch command for " role) {:role role})))
          (let [send-command (sh "tmux" "-S" socket "send-keys" "-t" (:session role-info) "-l" command)
                send-enter (sh "tmux" "-S" socket "send-keys" "-t" (:session role-info) "C-m")]
            (when-not (and (zero? (:exit send-command)) (zero? (:exit send-enter)))
              (throw (ex-info (str "failed to start lazy role " role)
                              {:command send-command :enter send-enter})))
            (fs/create-dirs (fs/parent marker))
            (spit (str marker) "started\n")
            (log! "started-lazy" role)
            (Thread/sleep 1000)))))))
      ...
      (do
        (doseq [recipient recipients]
          (let [role-info (get roles recipient)]
            ...
            (ensure-started! socket role-info)
