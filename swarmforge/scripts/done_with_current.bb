...
(defn run-lifecycle-hook! [event role-name mode]
  (let [result (process/sh {:continue true}
                           (str (fs/path script-dir "lifecycle_hook.bb"))
                           event
                           "SWARMFORGE_HOOK_ROLE" role-name
                           "SWARMFORGE_HOOK_RECEIVE_MODE" mode)]
...
(defn -main []
  (let [role-name (role)
    ...
    (run-lifecycle-hook! "pre-complete" role-name mode)
    (case mode
